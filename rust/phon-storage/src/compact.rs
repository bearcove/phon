...
impl Registry {
    ...
    pub fn primitive(&self, id: SchemaId) -> Option<Primitive> {
        self.primitives.get(&id).copied()
    }

    pub fn composite(&self, id: SchemaId) -> Option<&Schema> {
...
pub fn resolve(reg: &Registry, r: &SchemaRef) -> Result<Resolved> {
    match r {
...
pub fn alignment(p: Primitive) -> usize {
    match p {
...
pub use phon_schema::bytes::{pad_to, skip_pad};
...
pub fn min_wire_size_ref(reg: &Registry, rf: &SchemaRef) -> usize {
...
pub fn encode_primitive(value: &Value, p: Primitive, out: &mut Vec<u8>) -> Result<()> {
...
pub fn product(dimensions: &[u64]) -> Result<u64> {
    let mut p: u64 = 1;
    for &d in dimensions {
        p = p
            .checked_mul(d)
...
pub fn check_fixed_count(count: u64, min_wire: usize, remaining: usize) -> Result<()> {
...
pub fn decode_ref(r: &mut Reader, rf: &SchemaRef, reg: &Registry, depth: usize) -> Result<Value> {
    if depth > MAX_DEPTH {
...
pub fn decode_primitive(r: &mut Reader<'_>, p: Primitive) -> Result<Value> {
